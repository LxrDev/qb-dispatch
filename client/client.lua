...
if Config.Framework == "QBCore" then
        ...
        for k, v in pairs(Config.Jobs) do
            if Config.AllowedJobs[v].panic then
                if PlayerData.job.onduty then
                    ...
                    TriggerServerEvent('Lxr_dispatch:Server:SendAlert', job, text, coords, id)
                else
                    QBCore.Functions.Notify(LC['Not_Service'], 'error')
                end
            end
    ...
    RegisterCommand(Config.VehicleRob.command, function(args)
        ...
        if IsPedInAnyVehicle(ped, false) then
            ...
            QBCore.Functions.Notify(LC['Alert_Sent'])
            TriggerServerEvent("Lxr_dispatch:Server:SendVehRob", 'police', coords, model, plate, color, id)
        else
...
RegisterNetEvent("Lxr_dispatch:Client:SendAlert")
AddEventHandler("Lxr_dispatch:Client:SendAlert", function(text, coords, id)
    numcall = numcall + 1
    allcalls = allcalls + 1
...
RegisterNetEvent("Lxr_dispatch:Client:SendVehRob")
AddEventHandler("Lxr_dispatch:Client:SendVehRob", function(coords, model, plate, color, id)
    numcall = numcall + 1
    allcalls = allcalls + 1
